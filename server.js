...
io.on("connection", (socket) => {
  ...
  socket.on("login", async ({ username, password }, callback) => {
    ...
    if (!user || user.password !== password) return callback({ success: false });

    users[socket.id] = user.username;

    // Load last 100 messages and map to include avatars/colors/roles
  ...
  socket.on("setAvatar", async (url) => {
    ...
    await pool.query("UPDATE users SET avatar = $1 WHERE username = $2", [url, username]);
  ...
  socket.on("setColor", async ({ target, color }) => {
    ...
    await pool.query("UPDATE users SET color = $1 WHERE username = $2", [color, target]);
  });
  ...
  socket.on("chatMessage", async (msg) => {
    ...
    const userRes = await pool.query("SELECT avatar, role, color FROM users WHERE username = $1", [username]);
    const { avatar, role, color } = userRes.rows[0];

    const messageData = { username, msg, time, avatar, role, color };

    await pool.query("INSERT INTO messages (username, message, time) VALUES ($1, $2, $3)", [username, msg, time]);
